...
| 文件名 | 适用系统 |
|--------|---------|
| `WindowsMagnifier-x64.exe` | 64 位 Windows 10 / 11（主流电脑） |
| `WindowsMagnifier-x86.exe` | 32 位 Windows 10（老旧电脑） |
...
1. 运行 `WindowsMagnifier.exe`，放大镜窗口自动出现在每个显示器顶部
2. 移动鼠标，放大区域实时跟随
3. 右键点击放大镜窗口，选择**设置**可调整放大倍数和其他选项
4. 拖拽窗口底部边框可调整放大镜高度
5. 按 `Win + Alt + M` 可随时隐藏或显示放大镜
...
| 窗口高度 | 放大镜区域的像素高度（100–600） |
| 跟随鼠标指针 | 启用后放大区域跟随鼠标移动 |
| 跟随键盘输入 | 启用后打字时跟随文字光标位置 |
| 启动后最小化 | 开机启动时不显示放大镜窗口 |
...
Windows Magnifier is a desktop accessibility tool designed for people with **low vision**. It magnifies screen content in real time and displays it at the top of each monitor, making it easier for users with visual impairments to use their computer.
...
- **Global hotkey** — `Win + Alt + M` to instantly show or hide the magnifier
...
| File | System |
|------|--------|
| `WindowsMagnifier-x64.exe` | 64-bit Windows 10 / 11 (most modern PCs) |
| `WindowsMagnifier-x86.exe` | 32-bit Windows 10 (older PCs) |
...
1. Run `WindowsMagnifier.exe` — the magnifier window appears at the top of each monitor automatically
2. Move your mouse and the magnified area follows in real time
3. Right-click the magnifier window and select **Settings** to adjust zoom level and other options
4. Drag the bottom edge of the window to change the magnifier height
5. Press `Win + Alt + M` at any time to hide or show the magnifier
...
| Start minimized | Launch without showing the magnifier window |
